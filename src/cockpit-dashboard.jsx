...
// ─── Brand Logo ───────────────────────────────────────────────────────────────
// ⚠️ วางโค้ด Base64 ของโลโก้เดิมของคุณตรงนี้
const LOGO_B64 = "data:image/jpeg;base64,...(วางโค้ด Base64 ของโลโก้ที่นี่)...";
...
function CockpitSureModal({ qNo, branchId, data, jobIdx, onClose, onSuccess }) {
  ...
  // ✅ สลับกล้องใหม่ รองรับการสลับขณะถ่ายวิดีโอ (Real-time) โดยไม่หยุดบันทึก
  const switchCamera = async () => {
    ...
    try {
      if (phase === "recording" || phase === "paused") {
        // ขอแค่ Stream ภาพใหม่ ไม่เอาเสียงใหม่ เพื่อไม่ให้เสียงเก่าตัด
        ...
        if (stream) {
          // หยุดและถอดแทร็กวิดีโอเก่าออก
          stream.getVideoTracks().forEach(t => {
            t.stop();
            stream.removeTrack(t);
          });
          // เพิ่มแทร็กวิดีโอใหม่เข้าไปใน Stream ตัวเดิม
          newStream.getVideoTracks().forEach(t => stream.addTrack(t));
        }

        // โหลด Stream เดิมกลับเข้าจอภาพ
        if (videoRef.current) {
          videoRef.current.srcObject = stream;
          videoRef.current.play().catch(() => {});
        }
      } else {
        // ถ้าไม่ได้บันทึกอยู่ ก็เปิดกล้องใหม่ปกติ
          ...
          <div>
            <div style={{position:"relative",borderRadius:14,overflow:"hidden",
              ...
              <button onClick={switchCamera} style={{position:"absolute",top:10,left:10,
                ...
                padding:"6px 12px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",zIndex:10}}>
        ...
        {phase==="recording" && (
          <div>
            <div style={{position:"relative",borderRadius:14,overflow:"hidden",
              ...
              {/* ปุ่มสลับกล้องแบบ Real-time ตอนกำลังถ่าย */}
              <button onClick={switchCamera} style={{position:"absolute",top:10,left:10,
                background:"rgba(0,0,0,0.65)",border:"none",borderRadius:20,
                padding:"6px 12px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",zIndex:10}}>
                🔄 {facingMode==="environment"?"กล้องหน้า":"กล้องหลัง"}
