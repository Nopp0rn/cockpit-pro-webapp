...
const CLOUDINARY_PRESET = "cockpit_unsigned";import { useState, useEffect, useRef, useCallback } from "react";
...
const getActiveBranch = () => localStorage.getItem("activeBranch")||null;
const getActiveBranchName = () => localStorage.getItem("activeBranchName")||"";
const setActiveBranch = (id,name) => {
  localStorage.setItem("activeBranch",id);
  localStorage.setItem("activeBranchName",name||"");
  window.dispatchEvent(new CustomEvent("cockpitBranch",{detail:{id,name:name||""}}));
};
const JOB_TYPES = [
...
function AdminView() {
  ...
  const fetchOv = async () => {
    try {
      ...
      const d = await r.json();
      setOverview(d.overview || []);
      const saved=getActiveBranch(); if(saved) selectBranch(saved); else if(d.overview?.length) selectBranch(d.overview[0].branchId);
    } catch {}
    setLoading(false);
  };
  ...
  useEffect(() => { fetchOv(); }, []);
  useEffect(()=>{
    const h=(e)=>{ if(e.detail?.id) selectBranch(e.detail.id); };
    window.addEventListener("cockpitBranch",h);
    return ()=>window.removeEventListener("cockpitBranch",h);
  },[]);
  useEffect(() => {
...
export default function App() {
  const [tab,setTab]         = useState("staff");
  const [branches,setBranches] = useState([]);
  const [branchId,setBranchId] = useState(getActiveBranch);
  const [branchName,setBranchName] = useState(getActiveBranchName);
  ...
  useEffect(()=>{
    fetch(`${API}/api/admin/overview`).then(r=>r.json()).then(d=>{
      ...
      const name=list.find(b=>b.branchId===first)?.name||"";
      if(first){setBranchId(first);setBranchName(name);setActiveBranch(first,name);}
    }).catch(()=>{});
  },[]);

  const changeBranch=(id)=>{
    if(locked)return;
    const name=branches.find(b=>b.branchId===id)?.name||"";
    setBranchId(id);setBranchName(name);setActiveBranch(id,name);
  };
  const handleLock=()=>{
    if(locked){
  ...
  return(
    <div style={{fontFamily:"'Noto Sans Thai',sans-serif",background:"#F2F2EE",minHeight:"100vh"}}>
      ...
      <div style={{background:"#1A1A1A",
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",gap:6,
            ...
            {locked?(
              <span style={{flex:1,fontSize:14,fontWeight:900,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                🔒 {branchName}
              </span>
            ):(
              <select value={branchId||""} onChange={e=>changeBranch(e.target.value)}
                ...
                {branches.map(b=>(
                  <option key={b.branchId} value={b.branchId} style={{background:"#1A1A1A",color:"#fff"}}>
                    {b.name}
                  </option>
                ))}
              </select>
            ...
            <button onClick={handleLock} title={locked?"ปลดล็อค":"ล็อคสาขา"}
              style={{flexShrink:0,background:"none",border:"none",color:locked?"#6ee7b7":"rgba(255,255,255,.4)",fontSize:18,cursor:"pointer",padding:"0 2px"}}>
              {locked?"🔒":"🔓"}
            </button>
          </div>
        </div>
        ...
        <div style={{display:"flex"}}>
          {[{k:"staff",l:"👨‍🔧 พนักงาน"},{k:"admin",l:"📺 ข้อมูล"},{k:"history",l:"📊 สถิติ"},{k:"videos",l:"🎥 วีดีโอ"}].map(t=>(
            <button key={t.k} onClick={()=>setTab(t.k)} style={{
              flex:1,padding:"10px 2px",border:"none",background:"transparent",
              color:tab===t.k?"#FFE000":"rgba(255,255,255,.4)",fontSize:13,fontWeight:800,cursor:"pointer",
              borderBottom:tab===t.k?"3px solid #FFE000":"3px solid transparent",
      ...
      <div>
        {tab==="staff"?<StaffView/>:tab==="admin"?<AdminView/>:tab==="videos"?<VideoView/>:<HistoryView/>}
      </div>
    </div>
  );
}
